...
def test():
    ...
    transform_val = transforms.Compose(
        [transforms.Resize(224),
         ...
         transforms.ToTensor(),
         transforms.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225))])
    dataset_valn = MyDataset(root="./datalist/n-val.txt", transform=transform_val, loader=default_loader)
    dataLoader_valn = torch.utils.data.DataLoader(dataset=dataset_valn, batch_size=BATCH_SIZE, shuffle=False,
                                                  num_workers=NUMBER_WORK)
    dataset_valp = MyDataset(root="./datalist/p-val.txt", transform=transform_val)
    dataLoader_valp = torch.utils.data.DataLoader(dataset=dataset_valp, batch_size=BATCH_SIZE, shuffle=False,
                                                  num_workers=NUMBER_WORK)
    dataset_vals = MyDataset(root="./datalist/s-val.txt", transform=transform_val)
...
def test__move():
    ...
    net.eval()
    # 验证数据加载
    destpath = 'D:\\dataset\\image-classify\\classfy'
    root = './datalist/s.txt'
    ...
    for step_val, data in enumerate(dataLoader_val):
        ...
        for i in range(len(predicted.data)):
            if predicted.data[i].cpu() != b_y.data[i].cpu():
                ...
                if os.path.exists(destimg):
                    shutil.rmtree(destimg)
                shutil.move(os.path.join(imgs[i + step_val * BATCH_SIZE]), os.path.join(class_dirdest))
                # shutil.copy(os.path.join(imgs[i + step_val * BATCH_SIZE]), os.path.join(destpath))
    print('acc: %.6f ' % (acc_val_sum / float(step_val + 1)))
